		...
		implements Bc_phbIntface
{
	...
	public List<Phb> finds(int User_id, String select, String lot, int start, int limit)
	{
		...
		list.add(lot);
		if (-1 != User_id) {
			sql = sql + " and User_id=?"; list.add(Integer.valueOf(User_id));
		}
		if ("*".equals(select))
		...
		list.add(Integer.valueOf(start)); list.add(Integer.valueOf(limit));
	...
	public boolean update(int userid, String lot, double win, double xiaofei, int type)
	{
		...
		if (type == 0) {
			...
			sql = sql + " where Phb_type in(?,'all') and User_id=?"; list.add(lot); list.add(Integer.valueOf(userid));
	...
	public List<Bc_phb> findsPHB(int Userid, String type, int start, int limit)
	{
		...
		ArrayList list = new ArrayList();
		if (-1 != Userid) {
			sql = sql + " User_id=?"; list.add(Integer.valueOf(Userid));
		}
		if (StringUtils.isNotBlank(type)) {
			sql = sql + " and Phb_type=?"; list.add(type);
		}
		sql = sql + " order by Phb_total desc limit ?,?"; list.add(Integer.valueOf(start)); list.add(Integer.valueOf(limit));
