		...
		implements Bc_autoIntface
{
	...
	public List<Bc_auto> finds(String user, String lot, int status, String btime, String etime, int start, int limit)
	{
		...
		if (StringUtils.isNotBlank(user)) {
			sql = sql + " User_name like ?"; list.add(user);
		}
		if (StringUtils.isNotBlank(lot)) {
			sql = sql + " Auto_lot=?"; list.add(lot);
		}
		if (StringUtils.isNotEmptyAll(new String[] { btime, etime })) {
			sql = sql + " and Auto_time>? and Auto_time<?"; list.add(btime + " 00:00:00"); list.add(etime + " 24:00:00");
		}
		if (-1 != status) {
			sql = sql + " and Auto_status=?"; list.add(Integer.valueOf(status));
		}
		sql = sql + " order by Auto_time desc limit ?,?"; list.add(Integer.valueOf(start)); list.add(Integer.valueOf(limit));
